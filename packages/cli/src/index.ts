...
function buildProgram(): Command {
  ...
  program
    ...
    .option("--name <string>", "Name of the entity")
    .option("--description <string>", "Description of the entity")
    .option("--symbol <string>", "Symbol of the DAC tokens")
    .option("--commitment <uint256>", "Founder commitment (in treasury token)")
    .option("--allocation <uint256>", "Founder allocation of main tokens")
    .option("--max-supply <uint256>", "Max DAC main token supply")
    .option("--default-quorum <uint256>", "Default DAC quorum")
    .option("--dividends-enabled", "Enable dividends at DAC creation")
    .option("--defer-birth-role <address>", "Optional defer birth role address")

    .option("--dac-id <id>", "DAC id in indexer")
    .option("--cell-address <address>", "DACCell address")
    .option("--dac-address <address>", "Alias for DACCell address")
    .option("--dac <address>", "Alias for DACCell address")
    .option("--deal-id <id>", "Deal id in indexer")
    .option("--deal-address <address>", "Deal contract address")
    .option("--deal <address>", "Alias for deal contract address")
    .option("--deal-cell <address>", "DealCell contract address")
    .option("--id <id>", "Generic id alias for view commands")
    .option("--address <address>", "Generic address alias for view commands")
    .option("--limit <number>", "Alias for query limit")
    .option("--offset <number>", "Alias for query offset")
    ...
    .option("--stake-token <address>", "Deal stake token override")
